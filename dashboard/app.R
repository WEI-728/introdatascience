    ...
    tabItem(tabName = "MultipleFile",
            fluidRow(
              box(
                ...
                helpText("Please use the template provided!!! To get template:"),a("Click Here",href = "https://github.com/WEI-728/introdatascience/blob/main/dashboard/PersonalFinance.xlsx")
              ),
              tabBox(title = "Z",
                     id = "tabset3",
                     width = 12,
    ...
    tabItem(tabName = "Instructions",
            ...
            fluidRow(
              box(width = 12,
                  ...
                  column(width = 8,
                         tags$img(src = 'github.png',height = "400px",width = "800px")
...
server <- function(input, output) {
  ...
  output$savingTable2 <- renderDataTable(options=list(scrollX = TRUE,
    ...
    if(is.null(input$MultiExcel)){
      ...
      if(input$savingDataTable == "Both"){
        ...
        dataset2 <- suppressWarnings(as.data.frame(lapply(dataset,as.numeric)))
        dataset <- dataset %>% mutate("Total Actual" = rowSums(select(dataset2,contains("Actual"))))
        dataset <- dataset %>% mutate("Total Budget" = rowSums(select(dataset2,contains("Budget"))))
  ...
  output$Chart3 <- renderPlot({
    ...
    if(sum(select_if(dataset,is.numeric),na.rm = T) == 0)
      return()
    else{
      dataset <-
        dataset %>%
  ...
  output$Chart5 <- renderPlot({
    ...
    if(input$chartType4 == "Stacked Bar"){
      gg <- ggplot(dataset,aes(fill = forcats::fct_rev(name), y = value, x = Total, label=value)) +
        geom_bar(position="stack", stat="identity") +
        geom_text(aes(label=round(value,digits=2)),position = position_stack(vjust = 0.5)) + labs(x ="Criteria", y = "Amount")+
  ...
  output$summary <- renderUI({
    ...
    HTML(paste(br(),"Maximum monthly income : RM",a,"(",a1,")",br(),
               ...
               "Maximum monthly savings : RM",c,"(",c1,")",br(),
               "Total Savings this year : RM",d))
